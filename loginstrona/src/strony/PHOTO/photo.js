...
export const Photo = () => {
  ...
  const wypisTagu = tagList.map((element, index) => {
    return <Tag key={index} name={element.name} />;
  });
  ...
  const HandleSubmit = async (event) => {
    ...
    if (!newTagName.replace(/\s/g, "").length) {
      ...
      axios
        .post(
          API,
          { name: newTagName, photo_id: params.id },
          { headers: { Authorization: "Bearer " + Cookies.get("Ciastko") } }
        )
        .then((result) => {
          const API2 = `http://127.0.0.1:5000/api/photo/${params.id}/info`;
  ...
  const HandleSubmit2 = async (event) => {
    ...
    if (!deleteTagName.replace(/\s/g, "").length) {
      ...
      axios
        .post(
          API,
          { name: deleteTagName, photo_id: params.id },
          { headers: { Authorization: "Bearer " + Cookies.get("Ciastko") } }
        )
        .then((result) => {
          const API2 = `http://127.0.0.1:5000/api/photo/${params.id}/info`;
    ...
    <>
      ...
      <div className="stronaPhotoDisp">
        ...
        <div className="photoDisplay">
          {picture ? (
            <img className="fotka" src={picture} alt="skill issue" />
          ) : (
            <p>Loading...</p>
          )}
        </div>
        <div className="opisPhoto">
          <p className="ownerParagraph" style={{ padding: "5px" }}>
            Autor: {pictureData.owner}
          </p>
          <div className="tagDisplay" style={{ padding: "5px" }}>
            <p className="tagParagraph">Tagi:</p>
            <div className="tagList">{wypisTagu}</div>
          </div>
        </div>
      </div>
    </>
